...
const AUDIT_PATTERNS = {
  solidity: [
    ...
    { name: "unchecked_transfer_return", re: /^\s*(?:IERC20|ERC20)?[A-Za-z0-9_.()\s]*\.(?:transfer|transferFrom)\s*\([^)]*\)\s*;/, severity: "MEDIUM", why: "ERC20.transfer/transferFrom can return false (USDT historically) or revert (non-standard tokens). Wrap in require(...) or use SafeERC20.", verify: "Confirm return value is consumed by require/assert." },
  ...
  python: [
    { name: "sql_string_concat", re: /(?:execute|executemany)\s*\(\s*["'][^"']*["']\s*\+/, severity: "CRITICAL", why: "String concatenation in SQL = injection. Use ?-parameterized queries.", verify: "Confirm user input flows into the concatenation." },
